...
```{r}
MeshTime <- inla.mesh.1d(seq(min(sugar$yr), max(sugar$yr), length=5))
```
...
APred <- Diagonal(MeshSpace$n * MeshTime$n)
...
```{r}
# Estimation
effect <- list(Field, list(Intercept = rep(1, nrow(sugar)), X))


effectEst <- as.list(as.data.frame(cbind(1,1,X)))
effectEst[[1]] <- Field
effectEst[[2]] <- list(Intercept = rep(1, nrow(sugar)))
...
#StackEst <- inla.stack(data=list(tree = sugar$n_occ),
#                     A = AEstlist,
#                     effects = effectEst,
#                     tag="est")
...
```{r, warning = FALSE}
# Formula
form <- as.formula(paste("tree ~ 0 + Intercept +",paste0("bio",1:19,collapse = "+"),
                          " + f(field, model=SPDE, group=sugar$year, control.group=list(model='ar1', hyper=hSpec))"))
