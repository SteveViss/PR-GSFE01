---
title: "Spatiotemporal model with INLA assuming continuous time"
...
climateScale <- scale(climateSugar[,1:2])

# Order sugar using the year
sugarOrd <- sugar[order(sugar$yr),]

# Year information
year <- sugarOrd$yr - 1969 # The earliest year is 1970
...
```{r}
yearSeq <- seq(min(year) + 5, max(year) - 5, length=5)
MeshTime <- inla.mesh.1d(yearSeq)
```
...
A noteworthy difference between this results and the one obtained with the discrete time spatiotemporal model is the temporal autocorrelation parameter, which is much smaller here. Because we know that from one year to another there is a strong autocorrelation, we know that there is a strong autocorrelation across time. So, by having mesh points fairly spread out in time and because it is known that there is variability in the distribution of sugar maples through time, it is not overly surprising that there less temporal autocorrelation with this model.  
...
mapMean <- vector("list", length = 5)
map.025 <- vector("list", length = 5)
map.975 <- vector("list", length = 5)
...
Note that this code allows us to draw prediction maps only at the temporal mesh points.