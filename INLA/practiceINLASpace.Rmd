...
StackEst <- inla.stack(data=list(PreAbs = mite[,7]),
                     A = AEstlist,
                     effects = effectEst,
                     tag="est")

# Stack for prediction
StackPred <- inla.stack(data=list(PreAbs = NA),
                        A=APredlist,
                        effects=effectPred,
                        tag="pred")
...
```{r, warning = FALSE}
model <- inla(PreAbs ~ 0 + SubsDens + WatrCont + f(i, model = SPDE),
