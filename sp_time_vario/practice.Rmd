...
names(raw_air) <- c("station_id","time_id","quality")
...
saveRDS("./assest/data/pm10_germany.rds")
```

## Load the data

```{r}
pm10 <- readRDS("./assest/data/pm10_germany.rds")
head(pm10)
pm10_20090306 <- st_as_sf(subset(pm10, dates == "2009-03-06"), coords = c("lng", "lat"), crs = 4326)
mapview(pm10_20090306)
```
...
```{r}
air_vario <- variogram(PM10~1, spaceTime_air_2005_2010, width=20, cutoff = 200, tlags=0:5)
plot(air_vario)
plot(air_vario, wireframe = TRUE)
```
...
separableModel <- fit.StVariogram(air_vario, separableModel,
                                  method="BFGS")

# visual check on the fit
plot(air_vario, separableModel, wireframe =  TRUE)
plot(air_vario, wireframe = TRUE)
```
...
metricModel <- fit.StVariogram(air_vario, metricModel,
                                  method="BFGS")

plot(air_vario, metricModel, wireframe =  TRUE)
plot(air_vario, wireframe = TRUE)
```
