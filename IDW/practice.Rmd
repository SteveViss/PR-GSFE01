...
#### Quick reminder on the metrics

**Sum of squared prediction error**
$$\text{SSPE} = \sum_{i=1}^{m_k}\left(Z_i-\widehat{Z}^{(-k)}_i\right)^2$$

Using the vector `IDW_out`, we can compute the sum of squared prediction error (SSPE) for the IDW model (with $c = 2$)

TODO: GUILLAUME

```{r}
sum(hml_avg_tp$avg_tp - IDW_out)^2
```
...
parameter space (here $c$) in order to minimize the sum of squared prediction
error (SSPE). Once the minimum sum of squared prediction error (SSPE) found,
...
parameter as an argument and compute the SSPE with the Leave-one-out validation
routine (LOOCV).
...
cost_fun <- function(c){
  IDW_out <- vector(length = length(hml_avg_tp))
  for (i in 1:length(hml_avg_tp)) {
    IDW_out[i] <- idw(avg_tp ~ 1, hml_avg_tp[-i,], hml_avg_tp[i,], idp = c, debug.level = 0)$var1.pred
  }
  # Return the RMSE for the c value 
  return(sum(hml_avg_tp$avg_tp - IDW_out)^2)
}

# Set the initial c parameter
init_c <- 1
# Lauch optim function
# optim(init_c, cost_fun)
```
...
for (i in 1:nfolds) {
  ...
  m <- gstat(formula=avg_tp~1, locations=train, set=list(idp = 2))
  preds <- predict(m, newdata=test)$var1.pred

  # Assign the value to the RMSE vector
  vec_MSPE[i] <- sqrt(mean(preds - test$avg_tp)^2) 
}
...
## Using a weighted gaussian kernel


```{r}
# install.packages("spatialEco")
library("spatialEco")
sp.kde(hml_avg_tp, bw = 100, newdata = r_ref)
```