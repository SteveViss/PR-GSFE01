...
for (i in 1:nfolds) {
  ...
  preds <- predict(m, newdata=valid)$var1.pred

  # Assign the value to the MSPE vector
  vec_MSPE[i] <- mean((preds - test$avg_tp)^2) 
}
...
library("spatialEco")
test <- sp.kde(x= hml_avg_tp, y = hml_avg_tp$avg_tp,bw = 100, newdata = r_ref)
...
pt.kde <- sp.kde(x = meuse, bw = 1000, sta
