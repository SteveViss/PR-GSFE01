...
**The problem** - not easy to store and handle biological informations through all of these dimensions. 

--

**Storage formats**

1. Tabular data (CSV / Attributes Table in shp)
2. Hierarchical Data Format (NetCDF/HDF)
3. Database SQL, splitting the dataset into multiples tables
     - field stations metadata and locations
     - Samples attached to field stations
     - Observations attached to samples

**Take home** - Having the right format can help to solve that problem, and thus
...
The problem comes when you want to conduct spatio-temporal analysis. You might want
the date information to be recognize as a `Date` column type by R.

---
# Situation 1

Dates entries might have different format (hopefully not in the same column).
**Solution:** `as.Date()` will coerce the strings vector to a proper vector of dates.
...
composed using the elements shown in Table. 
...
If you have several dates formats in one column... deep breath... and have a look at
this nice package [`lubridate`](https://lubridate.tidyverse.org/).
...
???
Now, that the time have been briefly covered in R. It's time to visualize 
...
Let's play with the [OBIS database](https://obis.org/). I want to know where
...
```{r, fig.retina = 2, fig.align="center", fig.height = 6, fig.width = 10}
ggplot() + 
     geom_sf(data = filter(sf_halibut, year > 2006)) +
     facet_wrap(~year) 
```
...
If you have a large dataset with highly density points, it might appropriate to
summarize the informations into a count grid.
...
# Quick note on organizing `spacetime` data

<html><div style='float:left'></div><hr color='#EB811B' size=1px width=720px></html> 

---

# Spatio-temporal variograms with `gstat`
...
To do so, the quickest way to do it is to use the `stConstruct` function

---

# Spatio-temporal variograms with `gstat`

The data structure that we will use as an input for the function has to be in a
similar way (see below) where each row corresponds to a sample taken at a
specific time and a specific location.
