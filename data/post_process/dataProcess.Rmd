...
sites <- sites %>% 
  select(Latitude, Longitude, Site.Site) %>% 
  group_by(Site.Site) %>% 
...
hamilton_tp <- hamilton %>% filter(Variable == "TP") %>% group_by(Site.Site, Year.Année, JulianDay.JourJulian) %>% summarise(avg_tp=mean(Value.Valeur)) 
